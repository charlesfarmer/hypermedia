...
package ca.qc.collegeahuntsic.weblab6.servlet.taglib.message;
...
public class MessageTag extends AbstractTag {
	...
	public int doEndTag() throws JspTagException {
		...
		Locale locale = (Locale) session
				.getAttribute(Constants.LOCALE_ATTRIBUTE_NAME);
		if (locale == null) {
		...
		String languageResourceBundleName = Constants.LANGUAGE_RESOURCE_BUNDLE_NAME
				.replace('.', '/')
				+ "_"
				+ locale.getLanguage()
				+ "_"
		...
		if (classLoader.getResourceAsStream(languageResourceBundleName) == null) {
			languageResourceBundleName = Constants.LANGUAGE_RESOURCE_BUNDLE_NAME
					.replace('.', '/')
					+ "_"
					+ locale.getLanguage()
					+ ".properties";
		...
		try (InputStream inputStream = classLoader
				.getResourceAsStream(languageResourceBundleName)) {
