...
public class MarchandService extends Service implements IMarchandService {
	...
	@Override
	public void addMarchand(Session session, MarchandDTO marchandDTO)
			throws InvalidHibernateSessionException, InvalidDTOException,
			ServiceException {
		try {
			getMarchandDAO().add(session, marchandDTO);
	...
	@Override
	public MarchandDTO getMarchand(Session session, String idMarchand)
			throws InvalidHibernateSessionException,
			InvalidPrimaryKeyException, ServiceException {
		try {
			return (MarchandDTO) getMarchandDAO().get(session, idMarchand);
		} catch (DAOException daoException) {
			throw new ServiceException(daoException);
		}
	}
	...
	@Override
	public void updateMarchand(Session session, MarchandDTO marchandDTO)
			throws InvalidHibernateSessionException, InvalidDTOException,
			ServiceException {
		try {
			getMarchandDAO().update(session, marchandDTO);
		} catch (DAOException daoException) {
			throw new ServiceException(daoException);
		}
	}
	...
	@Override
	public void deleteMarchand(Session session, MarchandDTO marchandDTO)
			throws InvalidHibernateSessionException, InvalidDTOException,
			ServiceException {
		try {
			getMarchandDAO().delete(session, marchandDTO);
		} catch (DAOException daoException) {
			throw new ServiceException(daoException);
		}
	}
	...
	@SuppressWarnings("unchecked")
	@Override
	public void ajouterMarchand(Session session, MarchandDTO marchandDTO)
			throws InvalidHibernateSessionException, InvalidDTOException,
			MarchandAlreadyAddedException, ServiceException {
		List<MarchandDTO> marchands = (List<MarchandDTO>) marchandDTO
				.getMembreDTO().getMarchands();
		for (MarchandDTO marchand : marchands) {
			if (marchandDTO.getName().equals(marchand.getName())) {
				throw new MarchandAlreadyAddedException();
			}
		}
		addMarchand(session, marchandDTO);
	}
