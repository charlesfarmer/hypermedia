	...
	protected DAO(Class<?> beanClass) throws InvalidDTOClassException {
		...
		if (beanClass == null) {
			throw new InvalidDTOClassException(
					"La classe de DTO ne peut être null");
		}
	...
	protected static String getPrimaryKey(Connexion connexion,
		...
		try {
			...
			if (createPrimaryKeyRequest == null) {
				throw new InvalidPrimaryKeyRequestException(
						"La requête de création de clef primaire ne peut être null");
			}
			try (PreparedStatement createPreparedStatement = connexion
					.getConnection().prepareStatement(createPrimaryKeyRequest)) {
				try (ResultSet resultSet = createPreparedStatement.executeQuery()) {
					if (resultSet.next()) {
		...
		} catch (SQLException | InvalidHibernateSessionException | InvalidPrimaryKeyRequestException sqlException) {
