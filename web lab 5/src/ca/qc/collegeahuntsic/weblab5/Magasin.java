	...
	public static void main(String[] args) {
		...
		try {
			...
			try {
				...
				achatBean
						.setDateAchat(new Timestamp(System.currentTimeMillis()));
				...
				profilBean = gestionMagasin.getProfilService().add(
						gestionMagasin.getConnexion(), profilBean);
				clientBean = gestionMagasin.getClientService().add(
						gestionMagasin.getConnexion(), clientBean);
				stockBean = gestionMagasin.getStockService().add(
						gestionMagasin.getConnexion(), stockBean);
				produitBean = gestionMagasin.getProduitService().add(
						gestionMagasin.getConnexion(), produitBean);
				achatBean = gestionMagasin.getAchatService().add(
						gestionMagasin.getConnexion(), achatBean);
				lignePanierBean = gestionMagasin.getLignePanierService().add(
						gestionMagasin.getConnexion(), lignePanierBean);
				ligneFactureBean = gestionMagasin.getLigneFactureService().add(
						gestionMagasin.getConnexion(), ligneFactureBean);

				gestionMagasin.getProfilService().update(
						gestionMagasin.getConnexion(), profilBean);
				gestionMagasin.getClientService().update(
						gestionMagasin.getConnexion(), clientBean);
				gestionMagasin.getStockService().update(
						gestionMagasin.getConnexion(), stockBean);
				gestionMagasin.getProduitService().update(
						gestionMagasin.getConnexion(), produitBean);
				gestionMagasin.getAchatService().update(
						gestionMagasin.getConnexion(), achatBean);
				gestionMagasin.getLignePanierService().update(
						gestionMagasin.getConnexion(), lignePanierBean);
				gestionMagasin.getLigneFactureService().update(
						gestionMagasin.getConnexion(), ligneFactureBean);
				...
				// *
				gestionMagasin.getLignePanierService().delete(
						gestionMagasin.getConnexion(), lignePanierBean);
				gestionMagasin.getLigneFactureService().delete(
						gestionMagasin.getConnexion(), ligneFactureBean);
				gestionMagasin.getProduitService().delete(
						gestionMagasin.getConnexion(), produitBean);
				gestionMagasin.getStockService().delete(
						gestionMagasin.getConnexion(), stockBean);
				gestionMagasin.getAchatService().delete(
						gestionMagasin.getConnexion(), achatBean);
				gestionMagasin.getClientService().delete(
						gestionMagasin.getConnexion(), clientBean);
				gestionMagasin.getProfilService().delete(
						gestionMagasin.getConnexion(), profilBean);
